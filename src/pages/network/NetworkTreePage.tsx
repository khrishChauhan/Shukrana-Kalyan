 ...
 *
 * NetworkTreePage — Live Placement Tree visualization.
 * Calls get_network_tree RPC with type='placement'.
 * All node ranks display as "Member".
 */

import React, { useEffect, useState, useCallback } from 'react';
import { motion } from 'motion/react';
import { RefreshCw, GitBranch, Users, Activity, TrendingUp } from 'lucide-react';
...
const NodeCard = ({ node, isRoot }: { node: TreeNode; isRoot: boolean }) => {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className={`w-[148px] bg-white rounded-2xl p-3 flex flex-col items-center gap-2 shadow-md hover:shadow-lg transition-shadow
        ${isRoot ? 'ring-2 ring-[#ED8C32] ring-offset-2' : 'border border-gray-100'}`}
      >
        {/* Avatar */}
...
export default function NetworkTreePage() {
  ...
  const fetchTree = useCallback(async (depth: number, silent = false) => {
    ...
    try {
      ...
      const { data, error: rpcError } = await supabase.rpc('get_network_tree', {
        p_root_uuid: user.id,
        p_type: 'placement',
      });
  ...
  const totalDownline = flatData.filter(n => n.level > 0).length;
  const activeCount   = flatData.filter(n => n.status === 'ACTIVE' && n.level > 0).length;
  const leftCount     = flatData.filter(n => n.placement_side === 'left').length;
  const rightCount    = flatData.filter(n => n.placement_side === 'right').length;
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-bold text-[#232F46]">Community Network</h1>
          </div>
          <p className="text-sm text-gray-500">Your placement network tree — visualized in real-time</p>
        </div>
      ...
      <div className="flex gap-3 flex-wrap">
        <StatCard label="Total Downline" value={totalDownline} icon={<Users className="w-4 h-4 text-[#232F46]" />} color="bg-[#232F46]/10" />
        <StatCard label="Left Leg" value={leftCount} icon={<Activity className="w-4 h-4 text-blue-600" />} color="bg-blue-50" />
        <StatCard label="Right Leg" value={rightCount} icon={<Activity className="w-4 h-4 text-[#ED8C32]" />} color="bg-orange-50" />
        <StatCard label="Active Members" value={activeCount} icon={<TrendingUp className="w-4 h-4 text-emerald-600" />} color="bg-emerald-50" />
      </div>
      ...
      <div className="flex flex-wrap gap-4 text-xs text-gray-500 bg-white border border-gray-100 rounded-xl px-4 py-3 shadow-sm">
        ...
        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-emerald-400 inline-block" /> Active</span>
        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-amber-400 inline-block" /> Pending</span>
        <span className="flex items-center gap-1.5"><span className="inline-block w-4 h-px border-t-2 border-dashed border-gray-300" /> Empty Slot</span>
        <span className="flex items-center gap-1.5"><span className="inline-block w-3 h-3 rounded-sm ring-2 ring-[#ED8C32]" /> You (Root)</span>
        <span className="flex items-center gap-1.5"><span className="inline-block px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-[#232F46] text-white">L</span> Left Leg</span>
        <span className="flex items-center gap-1.5"><span className="inline-block px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-[#ED8C32] text-white">R</span> Right Leg</span>
      </div>
